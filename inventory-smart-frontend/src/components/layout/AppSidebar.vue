<template>
  <aside :class="[
            ...
            <ul class="flex flex-col gap-1">
              <li v-for="(item, index) in menuGroup.items" :key="item.name">
                <button v-if="item.subItems" @click="toggleSubmenu(groupIndex, index)" :class="[
                  ...
                  <span v-if="isExpanded || isHovered || isMobileOpen" class="menu-item-text truncate">{{ item.name
                    }}</span>
    ...
    <div class="pb-6">
      <button type="button" @click="handleLogout" class="menu-item group w-full menu-item-inactive">
